...
function combineTypes() {
  console.log('🔄 Combining TypeScript types into single generated.d.ts...');
  ...
  const files = fs.readdirSync(GENERATED_DIR)
    .filter(file => file.endsWith('.ts') && file !== 'generated.d.ts' && !file.includes('index'))
    .filter(file => !file.startsWith('i-'))
    .sort();

  if (files.length === 0) {
    console.warn('⚠️  No TypeScript files found in generated directory');
    return;
  }

  console.log(`📁 Found ${files.length} generated files:`, files.join(', '));
  ...
  for (const file of files) {
    ...
    for (const line of lines) {
      ...
      convertedLine = convertedLine.replace(/export class (\w+)/g, 'export interface $1');
    ...
    if (cleanLines.length > 0) {
      const title = file.replace('.ts', '').split('-').map(word =>
        word.charAt(0).toUpperCase() + word.slice(1)
      ).join(' ');

      // Skip InertiaSharedProps since Vue SFC compiler has issues with generics
  ...
  console.log('🧹 Cleaning up individual type files...');
  for (const file of files) {
    const filePath = path.join(GENERATED_DIR, file);
  ...
  console.log(`✅ Successfully combined types into ${OUTPUT_FILE}`);
  console.log(`📊 Generated file contains ${combinedContent.length} lines`);
...
try {
  ...
  console.error('❌ Error combining types:', error.message);
  process.exit(1);
}